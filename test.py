...
def CE_Net_Train():
    ...
    solver = MyFrame(model, dice_bce_loss, 2e-4,metrics)
    ...
    for epoch in range(1, total_epoch + 1):
        ...
        for img, mask in data_loader_iter_v:
            ...
            torchvision.utils.save_image(img[0, :, :, :], "test/img/"+ str(a) + ".jpg", nrow=1, padding=0)
            torchvision.utils.save_image(mask[0, :, :, :], "test/mask/"+ str(a) + ".jpg", nrow=1, padding = 0)
            torchvision.utils.save_image(pred[0, :, :, :], "test/pred/"+ str(a)  + ".jpg", nrow=1, padding=0)
